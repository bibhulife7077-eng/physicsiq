    ...
    <div style={{ fontSize, lineHeight: lineH, fontWeight: fontW, color: 'var(--t)' }}>
      {segments.map((seg, si) => {
        ...
        lines.forEach((line, li) => {
          ...
          // Assertion/Reason pattern: lines that are standalone statements
          // "Statement I:", "Statement II:", "Assertion:", "Reason:", "(A)", "(B)"
          const isStandalone =
            /^(Statement\s+(I+|[0-9]+)\s*[:\-]|Assertion\s*[:\-]|Reason\s*[:\-]|\([A-Da-d1-4]\)\s)/i.test(trimmed) ||
            /^[A-D]\.\s/.test(trimmed);

          if (isStandalone && paraLines.length > 0) flushPara();
...
// Question image component — looks up bundled image, renders nothing if missing.
function QuestionImage({ filename }) {
  if (!filename) return null;
  const src = IMAGE_MAP[filename];
  if (!src) {
    if (import.meta.env.DEV) console.warn(`QuestionImage: "${filename}" not found in src/images/`);
    return null;
  }
  return (
