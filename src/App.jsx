  ...
  return (
    <div style={{ fontSize, lineHeight: lineH, fontWeight: fontW, color: 'var(--t)' }}>
      {segments.map((seg, si) => {
          ...
          nodes.push(
            <div key={nodes.length} style={{ margin: '4px 0 12px', border: '1px solid var(--b)', borderRadius: 10, overflow: 'hidden' }}>
              {arBuffer.map((item, idx) => (
                <div key={idx} style={{ padding: '9px 12px', borderTop: idx === 0 ? 'none' : '1px solid var(--b)' }}>
                  <span style={{ fontFamily: 'var(--mono)', fontWeight: 700, color: 'var(--p2)', fontSize: 11, letterSpacing: 0.5, marginRight: 6 }}>
        ...
        const arRe = /^(Assertion|Reason|Statement\s*[-\s]?\s*(?:I{1,2}|[0-9]))\s*[:\-]\s*(.*)$/i;
        ...
        lines.forEach((line) => {
          ...
          if (arMatch) {
            flushPara(); flushList();
            arBuffer.push({ label: arMatch[1], text: arMatch[2] });
            return;
          }
