...
export default function App() {
  ...
  // Helper: count total questions across a year FOR CURRENT SUBJECT
  const yearQCount = (y) => {
    if (selSubject !== "Physics") return 1; // non-Physics: always show year as available
    let n = 0;
          ...
          {view === "years" && (
            <div className="sel-screen">
              ...
              <div className="year-grid">
                {ALL_YEARS.map(y => {
                  const isPhysics = selSubject === "Physics";
                  const hasData   = isPhysics ? (!!DB[y] && yearQCount(y) > 0) : true;
                  const qCount    = isPhysics ? yearQCount(y) : 0;
                  return (
          ...
          {view === "months" && selYear && (
            <div className="sel-screen">
              ...
              <div className="month-grid">
                {(() => {
                  // Physics: use real DB keys. Others: use standard JEE exam dates
                  const isPhysics = selSubject === "Physics";
                  const months = isPhysics
                    ? Object.keys(DB[selYear] || {})
                    : ["January", "April"];
                  return months.map(m => {
                    const shiftCount = isPhysics ? monthShiftCount(selYear, m) : 2;
                    const totalQ     = isPhysics ? Object.values(DB[selYear]?.[m] || {}).reduce((acc, s) => acc + (s.questions?.length || 0), 0) : 0;
                    return (
                      <div key={m} className="month-card has-data" onClick={() => goToShifts(m)}>
                        <div className="month-name">{m}</div>
                        <div className="month-info">
                          <div className="month-shifts">{shiftCount} shift{shiftCount > 1 ? "s" : ""}{isPhysics ? ` · ${totalQ} questions` : ""}</div>
          ...
          {view === "shifts" && selYear && selMonth && (
            <div className="sel-screen">
              ...
              <div className="shift-grid">
                {(() => {
                  const isPhysics = selSubject === "Physics";
                  // Physics: real shifts from DB. Others: 2 placeholder shifts
                  const entries = isPhysics
                    ? Object.entries(DB[selYear]?.[selMonth] || {})
                    : [["S1",{label:"Shift 1",date:"",questions:[]}],["S2",{label:"Shift 2",date:"",questions:[]}]];
