...
const progressKey = (subject, year, month, shift) => `${subject}_${year}_${month}_${shift}`;
...
//   pyq / {subject} / years / {year} / months / {month} / shifts / {shift}            → shift info
//   pyq / {subject} / years / {year} / months / {month} / shifts / {shift} / questions → questions
//   progress / {subject}_{year}_{month}_{shift}                                        → your scores
//
...
const seedFirestore = async () => {
  try {
    ...
    for (const [subject, years] of Object.entries(DB)) {
      for (const [year, months] of Object.entries(years)) {
        for (const [month, shifts] of Object.entries(months)) {
          for (const [shiftKey, shift] of Object.entries(shifts)) {
            // Save shift info (path now includes subject so Physics/Chemistry/Mathematics never collide)
            const shiftRef = doc(db, 'pyq', subject, 'years', year, 'months', month, 'shifts', shiftKey)
            batch.set(shiftRef, { label: shift.label, date: shift.date, totalQ: shift.totalQ, subject, year, month, shiftKey })
            // Save each question
            for (const q of shift.questions) {
              const qRef = doc(db, 'pyq', subject, 'years', year, 'months', month, 'shifts', shiftKey, 'questions', String(q.id))
              // Firestore rejects `undefined` field values, and numerical questions
              // have no `options` array — so only include it when it actually exists.
              const qData = { id: q.id, type: q.type, topic: q.topic, question: q.question, correct: q.correct, solution: q.solution, subject, year, month, shiftKey }
              if (q.options !== undefined) qData.options = q.options
    ...
    await batch.commit()
    console.log(`✅ Firestore seeded with ${total} questions`)
  } catch (err) {
...
const loadQuestionsFromFirestore = async (subject, year, month, shift) => {
  try {
    const colRef = collection(db, 'pyq', subject, 'years', year, 'months', month, 'shifts', shift, 'questions')
    ...
    return DB[subject]?.[year]?.[month]?.[shift]?.questions || []
  }
}
